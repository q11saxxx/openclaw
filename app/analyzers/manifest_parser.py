    ...
    def parse(self, path: str) -> dict:
        ...
        try:
            ...
            if skill_path.is_file():
                filename = skill_path.name.lower()
                # 如果是 Markdown 文件，尝试解析 Frontmatter
                if filename.endswith(".md"):
                    fm_result = self._parse_frontmatter(skill_path)
                    if fm_result:
                        result["manifest_files"].append(skill_path.name)
                        if fm_result.get("frontmatter_found"):
                            result["frontmatter_found"] = True
                            result["manifest_source"] = skill_path.name
                        self._merge_result(result, fm_result)
                
                # 如果是独立的 YAML 清单文件
                elif filename.endswith(".yaml") or filename.endswith(".yml"):
                    yaml_result = self._parse_yaml(skill_path)
                    if yaml_result:
                        result["manifest_files"].append(skill_path.name)
                        result["manifest_source"] = skill_path.name
