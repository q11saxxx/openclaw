...
class SkillService:
    ...
    async def save_uploaded_file(self, upload_file: UploadFile) -> dict:
        ...
        try:
            ...
            # 尝试解析 manifest 获取版本和作者信息
            try:
                parsed = self.parser.parse(str(dest))
                ...
                if manifest:
                    ...
                    # 使用快速检查的风险等级
                    manifest_updates['risk_level'] = quick_level
                    self.repo.update(saved.get('id'), manifest_updates)
                    saved = self.repo.get(saved.get('id'))
            except Exception:
                # manifest 解析失败不影响上传流程
                pass
    ...
    def delete_skill(self, skill_id: str) -> bool:
        """从仓储移除记录，并尽量删除磁盘上的上传文件（仅允许删除 upload_dir 下的路径）。"""
        rec = self.repo.get(skill_id)
        if not rec:
            return False
        ...
        if path_str:
            try:
                ...
                pass
        return self.repo.delete(skill_id) is not None
    ...
    def update_with_audit(self, skill_id: str, audit_result: dict) -> dict | None:
        ...
        updates = {
            "last_audit": datetime.datetime.utcnow().isoformat(),
            "last_audit_risk_level": summary.get("risk_level") if isinstance(summary, dict) else None,
            "last_audit_confidence": summary.get("confidence") if isinstance(summary, dict) else None,
